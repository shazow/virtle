...
Status: partly implemented (refs [#66](https://github.com/shazow/virtle/issues/66))

The packages described here exist: `units`, `vm`, `backend`, `backend/qemu`,
and `manifest`. What is not built yet is called out in
...
- `units` promoted out of `./internal`, rebased on `units.Bytes` with the
  binary multiplier constants; `units.MiB` stays for the MiB-denominated
  manifest surfaces.
- `vm`: `Spec` and its device types, `Guest`, `GuestWithCopy` with the
  `ArchiveFS` helper, `Term`/`TermOptions`.
- `backend`: `Backend`, `Instance`, the capability interfaces, and
  `backend.Shutdown`.
- `backend/qemu`: `BackendWithQGA`, whose instances start QEMU (with a
  virtiofs daemon per share), reach the guest through a `vm.Guest` adapter
  over QGA, and implement `backend.Suspender` and `backend.MemoryResizer`.
- `manifest.Load`, lowering a manifest to `(vm.Spec, backend.Backend)`.

Not built yet:

- The CLI still runs on `./internal/manager`; rewiring it onto these
  packages is the rest of phase 1.
- `backend.DeviceAttacher` and `backend.ConsoleProvider` are declared but
  the QEMU backend does not implement them; hotplug and the interactive
  console stay CLI-side for now.
- The `guest` package, the `virtle guest` subcommand, and
  `qemu.BackendWithGuest` (phase 3). `vm.GuestWithCopy` therefore has no
  implementation yet — the interface and `ArchiveFS` are in place for the
  daemon that will provide it.

Deviations from the design above, all additive:

- `vm.Disk` and `vm.Forward` carry a `ReadOnly` / `FromGuest` flag, which the
  manifest already expressed and the neutral types otherwise lost.
- `qemu.Config` grew the knobs a manifest-equivalent launch needs:
  `HostName`, `VirtiofsdBinary`/`VirtiofsdArgs`, `CIDRange`, `Balloon`.
- `manifest.LoadFile` is `Load` plus opening a path, since every caller was
  writing it.
- `manifest.Load` rejects the machine-describing manifest features the
  neutral `Spec` cannot carry yet (9p mounts, custom virtiofs socket paths,
  `write_files` chown, `workspace.mount_cwd`, guest-origin forward tunnels,
  the balloon controller) rather than dropping them silently. The
  session-orchestration sections named in D5 are ignored as designed.
