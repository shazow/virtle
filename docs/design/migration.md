...
Status: partly implemented, companion to [library-api.md](library-api.md)
...
Phases 1 and 2 have landed except for the CLI rewire: the packages exist and
the QEMU backend starts machines, but `virtle` itself still runs on
`./internal/manager`. Until that rewire happens the two paths share the
manifest resolver, the QEMU argv builder, and the launch primitives, so they
cannot drift.
