...
func TestDurationJSONRoundTrip(t *testing.T) {
	encoded, err := json.Marshal(GuestExecRequest{Path: "/bin/true", Timeout: manifest.Duration(90 * time.Second)})
	if err != nil {
	...
	if req.Timeout != manifest.Duration(2*time.Minute) {
	...
	if err := json.Unmarshal([]byte(`{"timeout":30}`), &req); err != nil || req.Timeout != manifest.Duration(30*time.Second) {
