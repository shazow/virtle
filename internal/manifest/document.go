...
type EgressInput struct {
	Reach   string              `json:"reach,omitempty" toml:"reach" jsonschema:"What the guest may reach besides the allow entries: internet (the default) is any public destination and nothing on the host or its networks; rules is only the allow entries; all is anything the host can reach."`
	Allow   []EgressRuleInput   `json:"allow,omitempty" toml:"allow" jsonschema:"Destinations the guest may reach, on the host's networks too, and which of them to inspect; with reach = rules nothing else is reachable."`
