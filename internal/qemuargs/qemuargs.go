// Package qemuargs builds the QEMU command line for a resolved manifest.
//
// It is shared by the CLI manager and the qemu backend, which lower different
// inputs onto the same resolved manifest before invoking QEMU.
package qemuargs
...
// BuildCommand renders the QEMU invocation for manifest, bound to the given
// vsock CID. Set incoming when the VM is being restored from saved state, so
// QEMU waits for the migration stream instead of booting.
func BuildCommand(manifest *manifest.Manifest, cid int, incoming bool) (*exec.Cmd, error) {
...
func buildQEMUArgs(qemu manifest.QEMU, cid int, incoming bool) ([]string, error) {
	...
	for i := 0; i < qemu.Hotplug.PCIEPorts; i++ {
		args = append(args, "-device", fmt.Sprintf("pcie-root-port,id=%s,bus=pcie.0,chassis=%d,slot=%d", hotplug.BusName(i), i+1, i+1))
	}
